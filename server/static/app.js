...
// 发行版 → 自托管 SVG 图标（Linux 专用，零外链）。platform 来自 gopsutil（如 "Ubuntu"/"CentOS Linux"/"Arch Linux"）
const DISTRO_MAP = [
  [/ubuntu/i, 'ubuntu'],
  [/debian/i, 'debian'],
  [/cent\s?os/i, 'centos'],
...
// 返回发行版官方 logo（仅图标、无文字，内联 SVG 跟随主题色）。匹配不到时回退通用 Linux 图标
function distroIcon(platform, os, title) {
  ...
  return `<svg class="distro-ico" viewBox="0 0 24 24" fill="currentColor" role="img" aria-label="${t}" title="${t}">${inner}</svg>`;
}

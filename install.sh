...
print_summary() {
  ...
  echo ""
  echo "  在其他机器安装 Agent:"
  echo "    docker run -d --network host --pid host \\"
  echo "      -e SERVER=ws://<服务器IP>:${PORT} \\"
  echo "      -e TOKEN=${AGENT_TOKEN} \\"
  echo "      -e INTERVAL=5 \\"
  echo "      -e UUID_FILE=/data/uuid \\"
  echo "      -v yufu-agent-data:/data \\"
  echo "      --name yufu-agent \\"
  echo "      --restart unless-stopped \\"
  echo "      ghcr.io/yufu-probe/yufu-agent:latest"
  echo ""
  echo "=========================================="
  echo ""
}

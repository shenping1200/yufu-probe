...
- **压力测试（虚拟机器）**：顶部「⚡ 压力测试」弹窗，自定义数量、国家/地区、操作系统、运行时长、在线率、流量档位、CPU/内存区间、开机年龄与分组名，一键在服务端生成一批"看着像真机器"的模拟节点（随机且实时抖动）；支持定时自动停止与手动「停止并清除」，仅移除本次生成的机器，绝不影响真实机器。每一项均可选「随机」由程序自动生成

...
### 2026-09-24 — 分组内自定义排序（拖拽 + 运行时间排序）
- **分组内排序偏好**：每个分组（含「全部」视图）可独立设置排序方式，互不影响。
- **运行时间排序**：列表视图「运行时间」列标题后新增排序按钮，点击在「默认 → 运行时间从长到短 → 短到长 → 默认」之间循环切换。
- **拖拽自定义排序**：卡片 / 列表视图均可拖拽机器重新排位，仅在「自定义」模式下生效（该模式下关闭虚拟滚动、整组渲染以便拖拽）。
- **后端持久化**：排序模式存于 `kv` 表的 `sort_mode:<分组>`，自定义顺序存于新增的 `group_sort` 表；新增 `GET/POST /api/group-sort` 接口（读取全部 / 保存）。
- **解决广播覆盖**：服务端每 2 秒广播全量快照会重置前端顺序，本版在每次渲染（含每帧快照后）按当前分组偏好重新施加排序，确保不抖动。
- **静态资源缓存策略**：`api.go` 静态路由现按扩展名设置缓存头——HTML（`/` / `*.html`）`Cache-Control: no-store`，JS/CSS `public, max-age=31536000, immutable`（靠 `?v=` 版本号破缓存）。此前无缓存头，更新前端后浏览器易读旧缓存导致界面不刷新。
